...
CBHDSYS = "https://github.com/PayXr/IyanStore"
...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("IyanStore").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("IyanStore").info(f"Invalid Git Command")
        ...
        LOGGER("IyanStore").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("IyanStore").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk Iyan-Ubot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n⚡ Powered By ~ @matstoreid ⚡"
    try:
        gruplog = await client.create_supergroup("⚡ Iyan-Logs ⚡", desc)
        if await in_heroku():
